...
for i in $(seq 1 $NUM_VALIDATORS); do
  docker run --rm \
    ...
    alpine sh -c '
      ...
      # === app.toml — use awk for robust section-based patching ===
      # awk handles any default value generated by "injectived init", unlike exact-match sed.
      # Key layout for Injective:
      #   [api]    → Cosmos REST on 10337 (moved away from 1317 to avoid conflict)
      #   [grpc]   → gRPC on 9900
      #   [evm-rpc]→ EVM JSON-RPC on 1317 / WS on 1318 (Injective-specific section name)
      awk '\''
        /^\[/ { current_section = $0 }
        current_section == "[api]" && /^enable /              { print "enable = true"; next }
        current_section == "[api]" && /^address /             { print "address = \"tcp://0.0.0.0:10337\""; next }
        current_section == "[api]" && /^enabled-unsafe-cors / { print "enabled-unsafe-cors = true"; next }
        current_section == "[grpc]" && /^address /            { print "address = \"0.0.0.0:9900\""; next }
        current_section == "[evm-rpc]" && /^enable /          { print "enable = true"; next }
        current_section == "[evm-rpc]" && /^address /         { print "address = \"0.0.0.0:1317\""; next }
        current_section == "[evm-rpc]" && /^ws-address /      { print "ws-address = \"0.0.0.0:1318\""; next }
        /^minimum-gas-prices /                                 { print "minimum-gas-prices = \"500000000inj\""; next }
        { print }
      '\'' "$APP" > "$APP.tmp" && mv "$APP.tmp" "$APP"

      # Ensure [evm-rpc] section exists — add it if injectived init did not generate it
      if ! grep -q "^\[evm-rpc\]" "$APP"; then
        printf "\n[evm-rpc]\nenable = true\naddress = \"0.0.0.0:1317\"\nws-address = \"0.0.0.0:1318\"\n" >> "$APP"
        echo "   Note: [evm-rpc] section appended (was absent from generated app.toml)"
      fi

      # Debug: print configured sections for visibility
      echo "--- app.toml [api] ---"
      awk "/^\[api\]/{f=1} f && /^\[/ && !/^\[api\]/{f=0} f{print}" "$APP"
      echo "--- app.toml [evm-rpc] ---"
      awk "/^\[evm-rpc\]/{f=1} f && /^\[/ && !/^\[evm-rpc\]/{f=0} f{print}" "$APP"
    '
